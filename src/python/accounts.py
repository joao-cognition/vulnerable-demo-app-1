...
def find_accounts_advanced(
    ...
    query = sql.SQL(
        ...
        sort_col=sql.Identifier(validated_sort_by),
        sort_dir=sql.SQL(validated_sort_dir),
    )
    ...
    try:
        ...
        cur.execute(
            query,
            (email_pattern, status, role, search_pattern, search_pattern, validated_limit, validated_offset),
        )
        return cur.fetchall()
    finally:
        conn.close()
